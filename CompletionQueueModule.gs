...
const CompletionQueueModule = (function () {
  ...
  function getQueueSheet_(spreadsheet) {
    ...
    try {
      ...
      console.error('CompletionQueueModule.getQueueSheet_: create failed', { error: err });
      return null;
    }
  }
  ...
  function scheduleRow_(sourceSheet, rowIndex, sheetRole, mapping, statusValue) {
    ...
    try {
      ...
      console.error('CompletionQueueModule.scheduleRow_: failed', {
        sourceSheet: sourceSheet.getName(),
        rowIndex: rowIndex,
        error: err,
      });
  ...
  function cancelRow_(sourceSheetName, rowSignature) {
    ...
    try {
      ...
      console.error('CompletionQueueModule.cancelRow_: failed', { error: err });
      return { success: false, error: String(err) };
    }
  }
  ...
  function removeQueueEntry_(queueRowIndex) {
    ...
    try {
      ...
      console.error('CompletionQueueModule.removeQueueEntry_: failed', {
