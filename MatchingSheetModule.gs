...
const MatchingSheetModule = (function () {
  ...
  function readMappings_(forceRefresh) {
    ...
    if (!spreadsheet) {
      ErrorLogModule.error('MatchingSheetModule', 'readMappings_', 'Spreadsheet unavailable', {});
      return [];
    }
    ...
    if (!sheet) {
      ErrorLogModule.error('MatchingSheetModule', 'readMappings_', 'Matching sheet not found', {
    ...
    try {
      ...
      ErrorLogModule.error('MatchingSheetModule', 'readMappings_', 'Read failed', { error: err });
      return [];
    }
  }
  ...
  function validateMappingsExist_() {
    ...
    if (errors.length > 0) {
      errors.forEach(function (validationError) {
        ErrorLogModule.warning('MatchingSheetModule', 'validateMappingsExist_', validationError, {
