...
const EmailSettingsModule = (function () {
  ...
  function readAllSettings_() {
    ...
    if (!spreadsheet) {
      console.error('EmailSettingsModule.readAllSettings_: spreadsheet unavailable');
      return [];
    }

    const sheet = SpreadsheetUtils.getSheetByName(spreadsheet, AppConfig.getEmailSettingsSheetName());
    if (!sheet) {
      console.error('EmailSettingsModule.readAllSettings_: email settings sheet not found');
      return [];
    }
    ...
    try {
      ...
      console.error('EmailSettingsModule.readAllSettings_: read failed', { error: err });
      return [];
    }
  }
