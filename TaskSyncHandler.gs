  ...
  function processMainEdit_(sheet, mapping, startRow, numRows) {
    ...
    for (var i = 0; i < numRows; i++) {
      ...
      try {
        ...
        ErrorLogModule.error('TaskSyncHandler', 'processMainEdit_', 'Row sync failed', {
  ...
  function processSubEdit_(sheet, mapping, startRow, numRows) {
    ...
    for (var j = 0; j < numRows; j++) {
      ...
      try {
        ...
        ErrorLogModule.error('TaskSyncHandler', 'processSubEdit_', 'Row sync failed', {
          sheetName: sheet.getName(),
          rowIndex: rowIndex,
          error: err,
        });
...
function handleSpreadsheetEditProduction_(e) {
  ErrorLogModule.setTriggerSource('onEdit');
  const lock = LockService.getScriptLock();
  var hasLock = false;
  try {
    hasLock = lock.tryLock(TaskSyncHandler.LOCK_TIMEOUT_MS || 30000);
    if (!hasLock) {
      ErrorLogModule.warning('TaskSyncHandler', 'handleSpreadsheetEditProduction_', 'Could not acquire lock', {
        triggerSource: 'onEdit',
      });
      return;
    }
    ...
    if (RenewalReminderHandler.shouldProcessEdit(e)) {
      RenewalReminderHandler.processEdit(e);
    }
  } catch (err) {
    ErrorLogModule.error('TaskSyncHandler', 'handleSpreadsheetEditProduction_', 'Handler failed', {
      error: err,
      triggerSource: 'onEdit',
    });
  } finally {
