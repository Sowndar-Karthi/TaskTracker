  ...
  function processEdit_(e) {
    ...
    for (var i = 0; i < numRows; i++) {
      ...
      try {
        ...
        ErrorLogModule.error('CompletionTaskHandler', 'processEdit_', 'Row failed', {
...
function processScheduledCompletionsProduction_() {
  ErrorLogModule.setTriggerSource('scheduled-completion');
  ...
  try {
    ...
    if (!hasLock) {
      ErrorLogModule.warning(
        ...
        'processScheduledCompletionsProduction_',
        'Could not acquire lock',
        { triggerSource: 'scheduled-completion' }
      );
      return;
    }
    CompletionQueueModule.ensureQueueSheet();
    ...
    ErrorLogModule.error('CompletionTaskHandler', 'processScheduledCompletionsProduction_', 'Failed', {
      error: err,
      triggerSource: 'scheduled-completion',
    });
  } finally {
