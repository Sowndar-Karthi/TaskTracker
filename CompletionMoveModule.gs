...
const CompletionMoveModule = (function () {
  ...
  function mapSubRowToCompletedRow_(subRowValues) {
    ...
    const account = subRowValues[SheetColumns.SUB.ACCOUNT - 1];
    if (mainTaskName) {
    ...
    if (account) {
      metaParts.push('Account: ' + account);
    }
  ...
  function moveRowToCompleted_(sourceSheet, sourceRowIndex, sheetRole, completedSheetName) {
    ...
    if (!completedSheet) {
      ErrorLogModule.error('CompletionMoveModule', 'moveRowToCompleted_', 'Completed sheet not found', {
    ...
    try {
      ...
      ErrorLogModule.error('CompletionMoveModule', 'moveRowToCompleted_', 'Move failed', {
        sourceSheet: sourceSheet.getName(),
        sourceRowIndex: sourceRowIndex,
        completedSheetName: completedSheetName,
        error: err,
    ...
    dueEntries
      ...
      .forEach(function (entry) {
        try {
          ...
          ErrorLogModule.error('CompletionMoveModule', 'processDueEntries_', 'Queue entry failed', {
            entry: entry,
            error: err,
          });
        }
