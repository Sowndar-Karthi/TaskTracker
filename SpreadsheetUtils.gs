...
const SpreadsheetUtils = (function () {
  ...
  function getSheetByName_(spreadsheet, sheetName) {
    if (!spreadsheet || !sheetName) {
      ErrorLogModule.warning('SpreadsheetUtils', 'getSheetByName_', 'Missing spreadsheet or sheet name', {
        sheetName: sheetName,
      });
      return null;
    }
    try {
      ...
      ErrorLogModule.error('SpreadsheetUtils', 'getSheetByName_', 'Failed to open sheet', {
        sheetName: sheetName,
        error: err,
      });
      return null;
  ...
  function getRowValues_(sheet, rowIndex, lastColumn) {
    ...
    try {
      ...
      ErrorLogModule.error('SpreadsheetUtils', 'getRowValues_', 'Read failed', {
        rowIndex: rowIndex,
