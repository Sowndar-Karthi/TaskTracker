...
## Shared Column Layout — Sub Task Trackers
...
| J | 10 | Status | Text | No | Synced from main task when matched; completion trigger | SubTaskSyncModule, CompletionMoveModule |
| K | 11 | Env | Text | No | Synced from main task when matched | SubTaskSyncModule |
| L | 12 | Estimated Effort | Number / Text | No | Synced from main task when matched | SubTaskSyncModule |
| M | 13 | Actual Effort | Number / Text | No | Synced from main task when matched | SubTaskSyncModule |
| N | 14 | Informed | Text | No | Synced from main task when matched | SubTaskSyncModule |
| O | 15 | Support Needed / Blocker | Text | No | Synced from main task when matched | SubTaskSyncModule |
| P | 16 | Latest Update / History | Text | No | Sub-task notes (not overwritten by sync) | Task modules |
| Q | 17 | Page Url | URL / Text | No | Synced from main task `Page Url` (col P) | SubTaskSyncModule |
| R | 18 | Functionality | Text | No | Synced from main task when matched | SubTaskSyncModule |
| S | 19 | Mail Subject | Text | No | Synced from main task when matched | SubTaskSyncModule |
| T | 20 | Internal Mail Subject | Text | No | Synced from main task when matched | SubTaskSyncModule |
| U | 21 | _(reserved)_ | — | No | Empty column in current layout | — |
| V | 22 | _(reserved)_ | — | No | Empty column in current layout | — |
| W | 23 | Gmail Thread ID | Text | No | Linked Gmail thread (not overwritten by sync) | Email integration |
| X | 24 | Google Task ID | Text | No | Linked Google Task (not overwritten by sync) | Google Tasks integration |
...
`Main Task Name`, `Ticket Id`, sub-task `Task`, `Status`, `Estimated Effort`, `Actual Effort`, `Latest Update / History`, `Gmail Thread ID`, `Google Task ID`
...
| `SheetColumns.SUB.PAGE_URL` | 17 | Q | Page Url | Sub task sheets |
| `SheetColumns.SUB.LATEST_UPDATE` | 16 | P | Latest Update / History | Sub task sheets |
| `SheetColumns.SUB.GMAIL_THREAD_ID` | 23 | W | Gmail Thread ID | Sub task sheets |
| `SheetColumns.SUB.GOOGLE_TASK_ID` | 24 | X | Google Task ID | Sub task sheets |
